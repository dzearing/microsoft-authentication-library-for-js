...
export function localStorageCache(ctx: ClientContext): void {
        ...
        crypto.subtle.deriveKey(
            {
                ...
                salt: nonce,
                hash: "SHA-256",
                info: new TextEncoder().encode(context(key)),
            },
            baseKey,
    ...
    const decrypt = async (key: string): Promise<string | null> => {
        ...
        try {
            return new TextDecoder().decode(
                await crypto.subtle.decrypt(
                    ZERO_IV,
                    await deriveKey(b64dec(o.nonce), key),
                    b64dec(o.data)
                )
            );
        } catch {
    ...
    const store: Store = {
        async init() {
            ...
            try {
                cookie = JSON.parse(
                    decodeURIComponent(raw!.slice(COOKIE.length + 1))
                );
            } catch {
